...
///
/// It replaced a 2.5pt seek track with a 20pt invisible hit strip. The ring
/// costs zero horizontal width — the scarce resource in a 360pt accessory —
...
struct PillProgressRing: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            Circle()
                // `strokeBorder` above insets by half the line width on its
                // own; `stroke` does not, so inset to match or the two arcs
                // sit on different radii.
                ...
                .stroke(theme.effectiveAccent(art: artColor),
                        style: StrokeStyle(lineWidth: PillLayout.ringLineWidth,
                                           lineCap: .round))
                // Trim starts at 3 o'clock; rotate so the arc grows clockwise
                // from the top.
